...
def generator(noise_dim=NOISE_DIM):
    ...
    # Replace "pass" statement with your code
    model = nn.Sequential(
        nn.Linear(noise_dim, 1024), nn.ReLU(),
...
def build_dc_classifier():
    ...
    model = nn.Sequential(
        ...
        nn.Conv2d(1, 32, 5, 1), nn.LeakyReLU(0.01),
        nn.MaxPool2d(2, 2),
        nn.Conv2d(32, 64, 5, 1), nn.LeakyReLU(0.01),
        nn.MaxPool2d(2, 2),
        nn.Flatten(), nn.Linear(4*4*64, 4*4*64), nn.LeakyReLU(0.01),
        nn.Linear(4*4*64, 1)
    )
    ############################################################################
    #                             END OF YOUR CODE                             #
