...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = dnext_h * (1 - torch.tanh(a) ** 2)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ht = h0
    for t in range(T):
        h[:, t, :], cache_t = rnn_step_forward(x[:, t, :], ht,
                                                 Wx, Wh, b)
        ht = h[:, t, :].clone()
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.transpose(1, 2), y, ignore_index=ignore_index, reduction='none').sum(-1).mean(-1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if self.cell_type == "rnn":
            self.img_to_h0 = nn.Linear(input_dim, hidden_dim)
            self.cell = RNN(wordvec_dim, hidden_dim)
        elif self.cell_type == "lstm":
            self.img_to_h0 = nn.Linear(input_dim, hidden_dim)
            self.cell = LSTM(wordvec_dim, hidden_dim)
        elif self.cell_type == "attn":
            self.img_to_h0 = nn.Conv2d(input_dim, hidden_dim, 1)
            self.cell = AttentionLSTM(wordvec_dim, hidden_dim)
        self.h_to_scores = nn.Linear(hidden_dim, vocab_size)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "rnn" or self.cell_type == "lstm":
            img = img.mean(-1).mean(-1)
        h0 = self.img_to_h0(img)
        h = self.cell(x, h0)
        scores = self.h_to_scores(h)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "rnn" or self.cell_type == "lstm":
            img = img.mean(-1).mean(-1)
        A = self.img_to_h0(img)
        if self.cell_type == "attn":
            h = A.mean(dim=(2, 3))
        else:
            h = A
        x = self.embed(self._start).unsqueeze(0).repeat(N, 1)
        c = h
        attn_weights_all = []
        for t in range(max_length):
            if self.cell_type == 'rnn':
                h = self.cell.step_forward(x, h)
            elif self.cell_type == 'lstm':
                h, c = self.cell.step_forward(x, h, c)
            elif self.cell_type == 'attn':
                attn, attn_weights = dot_product_attention(h, A)
                attn_weights_all.append(attn_weights.unsqueeze(1))
                h, c = self.cell.step_forward(x, h, c, attn)
            scores = self.h_to_scores(h)
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        ct = c0
        for t in range(T):
            hn[:, t, :], ct = self.step_forward(x[:, t, :], ht, ct)
            ht = hn[:, t, :].clone()
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    # Replace "pass" statement with your code
    A_flat = A.view(N, H, -1)
    attn_weights = torch.sum(prev_h.unsqueeze(-1) * A_flat, dim=1) / (H ** 0.5)
    attn_weights = F.softmax(attn_weights, dim=-1)
    attn = torch.sum(A_flat * attn_weights.unsqueeze(1), dim=-1)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        ct = c0
        for t in range(T):
            attn = dot_product_attention(ht, A)[0]
            hn[:, t, :], ct = self.step_forward(x[:, t, :], ht, ct, attn)
            ht = hn[:, t, :].clone()
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
