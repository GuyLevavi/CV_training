...
def prepocess_input_sequence(
    ...
    for word in words:
        if word in spc_tokens:
            ...
            sub_list = [token_dict[str_num] for str_num in word]
            out.extend(sub_list)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_single(
    ...
    for i, q in enumerate(query):
        ...
        for j, k in enumerate(key):
            scores[j] = torch.dot(q, k)
        scores = torch.softmax(scores / torch.sqrt(torch.tensor(M)), dim=-1)
        out[i] = scores @ value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(K):
        scores = torch.zeros(N, K)
        q = query[:, i, :].unsqueeze(1)
        for j in range(K):
            k = key[:, j, :].unsqueeze(1)
            scores[:, j] = torch.bmm(q, k.permute(0,2,1)).squeeze(-1).squeeze(-1)
        scores = torch.softmax(scores / torch.sqrt(torch.tensor(M)), dim=-1)
        o = torch.bmm(scores.unsqueeze(-1).permute(0,2,1), value).permute(0,2,1).squeeze(-1)
        out[:, i, :] = o
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights = torch.bmm(query, key.permute(0,2,1))
    if mask is not None:
        ##########################################################################
        ...
        # Replace "pass" statement with your code
        weights[mask] = -1e9
    weights_softmax = torch.softmax(weights / torch.sqrt(torch.tensor(M)), dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # Replace "pass" statement with your code
        import math
        bound = lambda x: math.sqrt(6 / (dim_in + x))
        self.q = nn.Linear(dim_in, dim_q)
        self.k = nn.Linear(dim_in, dim_q)
        self.v = nn.Linear(dim_in, dim_v)
        torch.nn.init.uniform_(self.q.weight, a=-bound(dim_q), b=bound(dim_q))
        torch.nn.init.uniform_(self.k.weight, a=-bound(dim_q), b=bound(dim_q))
        torch.nn.init.uniform_(self.v.weight, a=-bound(dim_v), b=bound(dim_v))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.heads = nn.ModuleList([
            SelfAttention(dim_in, head_dim, head_dim)
            for _ in range(num_heads)
        ])
        self.linear = nn.Linear(dim_out, dim_in)
        bound = math.sqrt(6 / (dim_out + dim_in))
        torch.nn.init.uniform_(self.linear.weight, a=-bound, b=bound)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out_list = [head(query, key, value, mask) for head in self.heads]
        out_cat = torch.cat(out_list, dim=-1)
        y = self.linear(out_cat)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        bound = lambda dim_in, dim_out: math.sqrt(6 / (dim_in + dim_out))
        torch.nn.init.uniform_(self.fc1.weight,
                               a=-bound(inp_dim, hidden_dim_feedforward),
                               b=bound(inp_dim, hidden_dim_feedforward))
        torch.nn.init.uniform_(self.fc1.weight,
                               a=-bound(hidden_dim_feedforward, inp_dim),
                               b=bound(hidden_dim_feedforward, inp_dim))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.mha = MultiHeadAttention(num_heads, emb_dim, num_heads * emb_dim)
    ...
    def forward(self, x):
        ...
        # inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \
        # - feedforward - out3 - layer_norm(out3 + out2) - dropout - out
        x = self.do(self.ln1(self.mha(x, x, x) + x))
        y = self.do(self.ln2(self.ff(x) + x))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # Replace "pass" statement with your code
        self.attention_self = MultiHeadAttention(num_heads, emb_dim, num_heads * emb_dim)
        self.attention_cross = MultiHeadAttention(num_heads, emb_dim, num_heads * emb_dim)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.arange(K) / K
    y = y.unsqueeze(1).repeat(1, M).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    p = torch.arange(0, K, dtype=torch.float).unsqueeze(1)  # Shape: [K, 1]
    i = torch.arange(0, M, dtype=torch.float).unsqueeze(0)  # Shape: [1, M]

    div = 10000 ** torch.floor(((i[:,::2]) / M))

    pe = torch.zeros(K, M)
    pe[:, 0::2] = torch.sin(p/div)
    pe[:, 1::2] = torch.cos(p / div[:, : (M // 2)])
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        q_enc_out = self.encoder(q_emb_inp)
        mask = get_subsequent_mask(ans_b[:,:-1])
        dec_out = self.decoder(a_emb_inp, q_enc_out, mask)
        dec_out = dec_out.flatten(start_dim=0, end_dim=1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
